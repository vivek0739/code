...
map<int, string> wordInvIndex;
int wordIndexCnt = 0;
int termDocumentFrequency[50000][15];
...
void printRoots(int ind, string tokenFile) {
	...
	for(it = rootWords[ind].begin(); it != rootWords[ind].end(); it++) {
		if(!wordIndex[it -> first]) {
			wordIndex[it -> first] = wordIndexCnt + 1;
			wordIndexCnt++;
			wordInvIndex[wordIndexCnt] = it -> first;
		}	
...
double calculateSimilarity(int ind1, int ind2) {
	int i;
	double magnitudeDoc1 = 0;
	double magnitudeDoc2 = 0;
	double similarity = 0;
	for(i = 1; i <= wordIndexCnt; i++) {
		similarity += termDocumentFrequency[i][ind1] * termDocumentFrequency[i][ind2];
		magnitudeDoc1 += termDocumentFrequency[i][ind1] * termDocumentFrequency[i][ind1];
		magnitudeDoc2 += termDocumentFrequency[i][ind2] * termDocumentFrequency[i][ind2];
	}
	similarity = similarity / (sqrt(magnitudeDoc1) * sqrt(magnitudeDoc2));
	return similarity;
}
string fileNames[15];
int main() {
	...
	while(numFiles--) {
		...
		if(!exists(tokenFile)) {
			...
			string stemWord;
			int cnt;
			while(fin >> stemWord >> cnt) {
				if(!wordIndex[stemWord]) {
					wordIndex[stemWord] = wordIndexCnt + 1;
					wordIndexCnt++;
					wordInvIndex[wordIndexCnt] = stemWord;
				}
				rootWords[numFiles][stemWord] = cnt;
			}
		}
	}
	numFiles = temp;
	while(numFiles--) {
		//cout << numFiles << endl;
		map<string, int>::iterator it;
		for(it = rootWords[numFiles].begin(); it != rootWords[numFiles].end(); it++) {
			termDocumentFrequency[wordIndex[it -> first]][numFiles] += it -> second;
		}				
	}
	numFiles = temp;
	int i, j;
	for(i = numFiles - 1; i >= 0; i--) {
		for(j = i - 1; j >= 0; j--) {
			cout << "Similarity between doc " << numFiles - i << " doc " << numFiles - j << " " << calculateSimilarity(i, j) << endl;
		}
	}			
	return 0;
}

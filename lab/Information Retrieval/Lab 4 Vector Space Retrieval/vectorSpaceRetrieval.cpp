...
int main() {
	...
	cout << "Enter the number of key words: ";
	int numKeyWords;
	cin >> numKeyWords;
	...
	while(numKeyWords--) {	
		cin >> query;
		stemfile(query.c_str());
		string str(s);
		queryVector[s]++;
	}
	pair<pair<string, string>, int> temp = getWords(query);
	vector<pair<double, string> > similarityList;
	...
	while(numFiles--) {
		...
		double magnitudeDoc = 0;
		map<string, int>::iterator it;
		for(it = rootWords[numFiles].begin(); it != rootWords[numFiles].end(); it++) {
			magnitudeDoc += (it -> second) * (it -> second);
			if(queryVector[it -> first]) {
				magnitudeQuery += (queryVector[it -> first] * queryVector[it -> first]);
				similarity += (it -> second) * queryVector[it -> first];
			}
		}
		if(similarity != 0.0) {
			similarity = similarity / (sqrt(magnitudeQuery) * sqrt(magnitudeDoc));
			similarityList.push_back(make_pair(similarity, inputFile));
		}						
	}
	sort(similarityList.begin(), similarityList.end());
	reverse(similarityList.begin(), similarityList.end());
	int i;
	for(i = 0; i < similarityList.size(); i++) {
		cout << similarityList[i].first << " " << similarityList[i].second << endl;
	}
	return 0;
}

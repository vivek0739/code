...
int main() {
	...
	while(iss >> word) {
		for(it = classDocs.begin(); it != classDocs.end(); it++) {
			totalclass+=classDocs[it->first].size();
			...
			classProb[it -> first] *= (double) (sumTF +1)/ (sumDT+tb);
			//cout << classProb[it -> first] << endl;
		} 
	}
	for(it = classDocs.begin(); it != classDocs.end(); it++) {
		classProb[it->first]*=(double)(classDocs[it->first].size())/totalclass;
	}
	string answer;
	double maxval = -1;
	for(it = classDocs.begin(); it != classDocs.end(); it++) {
